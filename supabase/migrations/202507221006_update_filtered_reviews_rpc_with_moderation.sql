-- supabase/migrations/20250722_update_filtered_reviews_rpc_with_moderation.sql

-- This migration updates the `get_filtered_reviews` RPC function
-- to correctly filter reviews based on their moderation_status.

-- Drop the old RPC function to replace it with the new one
...
RETURNS TABLE (
    ...
    moderation_status TEXT -- ADDED: moderation_status to the return type
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    -- ADDED: Moderation status filtering
    -- Users can always see their own reviews. For others, only 'approved' reviews are visible.
    _where_clause := _where_clause || format(' AND (r.user_id = %L OR r.moderation_status = %L)', _current_user_id, 'approved');
    ...
    RETURN QUERY EXECUTE format('
            ...
            (
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        ''moderation_status'', rc.moderation_status -- Include moderation_status for comments
                ...
                AND (rc.user_id = %L OR rc.moderation_status = %L) -- ADDED: Filter comments by moderation status
            ) AS review_comments,
            r.moderation_status -- ADDED: moderation_status to the main review selection
        FROM
            public.reviews r
        JOIN
        ...
        ORDER BY
            %s
        LIMIT %L OFFSET %L
    ', _current_user_id, _current_user_id, _current_user_id, _current_user_id, 'approved', _where_clause, _order_by_clause, p_limit, _offset);
END;
$$;
