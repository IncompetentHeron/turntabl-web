...
-- including by artist, release year/decade, new releases,
-- and a Bayesian average for rating-based sorting.
...
BEGIN
    ...
    CASE p_sort_by
        ...
        WHEN 'popular_all_time' THEN
            _order_by_clause := 'COALESCE(review_stats.review_count, 0) DESC, a.created_at DESC';
        WHEN 'popular_this_week' THEN
            -- This requires counting reviews from the last 7 days.
            -- For simplicity in this function, we'll use the existing `get_popular_albums_by_reviews`
            -- or a similar logic if it were integrated. For now, we'll use total review count.
            -- A more complex solution would involve a subquery to count recent reviews.
            -- For now, we'll default to overall popularity if 'popular_this_week' is chosen here.
            _order_by_clause := 'COALESCE(review_stats.review_count, 0) DESC, a.created_at DESC';
        WHEN 'top_rated' THEN
...
GRANT EXECUTE ON FUNCTION public.get_filtered_albums(TEXT, INT, INT, TEXT, INT, INT, BOOLEAN) TO anon, authenticated;
